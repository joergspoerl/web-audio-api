...
export class HomePage implements OnInit {
  ...
  @ViewChild('myplayer1') playerElement;
  ...
  //   //
    ...
    var hBand = this.audioContext.createBiquadFilter();
    hBand.type = "lowshelf";
    hBand.frequency.value = bandSplit[0];
    hBand.gain.value = gainDb;

    var hInvert = this.audioContext.createGain();
    hInvert.gain.value = -1.0;

    var mBand = this.audioContext.createGain();

    var lBand = this.audioContext.createBiquadFilter();
    lBand.type = "highshelf";
    lBand.frequency.value = bandSplit[1];
    lBand.gain.value = gainDb;

    var lInvert = this.audioContext.createGain();
    lInvert.gain.value = -1.0;

    this.bufferSource.connect(lBand);
    this.bufferSource.connect(mBand);
    this.bufferSource.connect(hBand);
    ...
    this.lGain = this.audioContext.createGain();
    this.mGain = this.audioContext.createGain();
    this.hGain = this.audioContext.createGain();

    lBand.connect(this.lGain);
    mBand.connect(this.mGain);
    hBand.connect(this.hGain);

    var sum = this.audioContext.createGain();
    this.lGain.connect(sum);
    this.mGain.connect(sum);
    this.hGain.connect(sum);
    sum.connect(this.audioContext.destination);
